...
app.post("/register", async (req, res) => {
  ...
  try {
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      res
        .status(200)
        .send("Registered successfully, check your email for the OTP");
    });
  } catch (error) {
...
app.post("/tasks", async (req, res) => {
  const { title, description, dueDate, priority, category, labels, userEmail } =
    req.body;
  ...
  try {
    ...
    overdueTasks.forEach(async (task) => {
      ...
      const mailOptions = {
        ...
        text:
          `Reminder: Your task "${task.title}" is overdue. Your credibility points will be deducted.` +
      ...
      transporter.sendMail(mailOptions, async (error, info) => {
        if (error) {
          console.error(
            "Error sending overdue task reminder email:",
            error.message
          );
        } else {
